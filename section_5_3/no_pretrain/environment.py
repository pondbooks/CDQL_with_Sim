...
class ENVIRONMENT:
    ...
    def run(self):        
        ...
        for learning_step in range(MAX_STEP): 
            ...
            noise = max((400-learning_step),0.0)/400*0.1*np.random.normal()
            action = action + noise
            ...
            if len(memory) > self.args.batch_size:
                ...
                print('update t='+str(learning_step))
        ...
        fig.savefig('no_pretrain.eps', bbox_inches="tight", pad_inches=0.05) 
        fig.savefig('no_pretrain.png', bbox_inches="tight", pad_inches=0.05) 
