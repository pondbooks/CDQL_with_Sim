...
class BRAIN:
    ...
    def decide_action(self, state, weight):
        ...
        P_inv = (weight_1 * P_1 + weight_2 * P_2 + weight_3 * P_3 + weight_4 * P_4)**(-1) #(1,1)

        action = (weight_1 * (P_1 @ action_1) + weight_2 * (P_2 @ action_2) + weight_3 * (P_3 @ action_3) + weight_4 * (P_4 @ action_4)) 
    ...
    def compute_Q_value(self, state, action):
        ...
        Q_vector = np.array([[Q_1, Q_2, Q_3, Q_4]])
    ...
    def compute_next_value(self, next_state, weight):
        ...
        Q_next_vector = np.array([[Q_1, Q_2, Q_3, Q_4]]) #(1,4)
